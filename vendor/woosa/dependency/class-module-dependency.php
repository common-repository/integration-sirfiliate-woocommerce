...
class Module_Dependency {
   ...
   public static function is_check_passed($output = true, $go_back = false){
      ...
      try {
         ...
         $link    = sprintf(__('%sGo back.%s', 'integration-sirfiliate-woocommerce'), '<a href="' . admin_url('plugins.php') . '">', '</a>');
   ...
   protected static function check_php_version(){
      ...
      if(version_compare(phpversion(), self::php_version(), '<')){
         throw new \Exception(sprintf(
            __('The server must have at least %s installed.', 'integration-sirfiliate-woocommerce'),
   ...
   protected static function check_php_extensions(){
      ...
      foreach(self::php_extensions() as $slug => $name){
         if(!in_array($slug, $active)){
            throw new \Exception(sprintf(
               __('This plugin requires %s extension to be installed on the server.', 'integration-sirfiliate-woocommerce'),
               "<b>{$name}</b>"
            ));
         }
      }
   ...
   */
   ...
   protected static function check_wp_version(){
      ...
      if(version_compare(get_bloginfo('version'), self::wp_version(), '<')){
         throw new \Exception(sprintf(
            __('This plugin requires at least %s!', 'integration-sirfiliate-woocommerce'),
   ...
   protected static function check_wp_plugins(){
      ...
      foreach(self::wp_plugins() as $path => $item){
         ...
         $message = sprintf(
            __('This plugin requires at least %s to be installed and active.', 'integration-sirfiliate-woocommerce'),
    ...
    * @return array
    * */
   protected static function get_active_wp_plugins(){
